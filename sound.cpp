...
{
	{ "data/BGM/タイトル.wav", -1 },			// BGM0
	{ "data/BGM/ステージ1.wav", -1 },			// BGM1
	{ "data/SE/ジャンプ/ジャンプ.wav", 0 },		// SE0
};
...
{
	// 一時停止
	for (int nInd = 0; nInd < SOUND_LABEL_MAX; nInd++)
	{
		if (m_apSourceVoice[nInd])
		{
			// 一時停止
			m_apSourceVoice[nInd]->Stop(0);

			// ソースボイスの破棄
			m_apSourceVoice[nInd]->DestroyVoice();
			m_apSourceVoice[nInd] = NULL;

			// オーディオデータの開放
			free(m_apDataAudio[nInd]);
			m_apDataAudio[nInd] = NULL;
		}
	}
